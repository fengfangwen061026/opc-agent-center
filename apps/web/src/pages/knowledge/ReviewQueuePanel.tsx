          ...
          return (
            <GlassCard key={note.path} className="opc-review-card" variant="soft">
              <div className="opc-review-card__header">
                ...
                <StatusPill
                  status={status === 'done' ? 'connected' : 'running'}
                  label={status === 'done' ? '已处理' : '待处理'}
                />
              </div>
              <p>
                {note.content
              ...
              <div className="opc-tag-list">
                {(Array.isArray(note.frontmatter?.tags) ? note.frontmatter.tags : [])
                  .slice(0, 5)
                  .map((tag) => (
              ...
              <div className="opc-drawer-actions">
                ...
                <LiquidButton
                  ...
                  onClick={() =>
                    window.open(
                      `obsidian://open?vault=obsidian-vault&file=${encodeURIComponent(note.path)}`,
                    )
                  }
                >
                  在 Obsidian 中打开
